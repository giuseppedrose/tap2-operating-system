-- Tap2 Operating System — Supabase Schema
...
insert into partners (name, role) values
  ('Giuseppe', 'Founder'),
  ('Dorian', 'Sales'),
  ('Joaquin', 'Sales - Spain'),
  ('Jonathan', 'Sales - Colombia'),
  ('Carlo', 'Sales - Italy'),
  ('Niels', 'Sales'),
  ('Qubico Studio', 'Agency Partner'),
  ('Other', 'Other')
...
create table if not exists customers (
  ...
  city text,
  industry text,
  source text references gtm_sources(name),
  partner_owner text references partners(name),
  stripe_customer_id text unique,
  hubspot_company_id text unique,
  status text not null default 'active' check (status in ('active', 'trial', 'churned', 'paused')),
...
create table if not exists outbound_campaigns (
  ...
  market text,
  segment text,
  owner text references partners(name),
...
create table if not exists campaign_leads (
  ...
  city text,
  segment text,
  status text not null default 'new' check (status in ('new', 'contacted', 'replied', 'positive', 'meeting', 'not_interested', 'unsubscribed')),
...
-- (add RLS policies per your auth setup)
