  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <KpiCard title="Monthly Burn" value={`€${mockCashData.monthlyBurn.toLocaleString()}`} subvalue="gross burn" icon={<TrendingDown className="h-5 w-5" />} />
        <KpiCard
          title="Runway"
        ...
        />
        <KpiCard title="Monthly Revenue" value="€1,401" subvalue="MRR covering burn" icon={<CreditCard className="h-5 w-5" />} />
      </div>

      {/* Burn breakdown */}
      <div className="grid grid-cols-3 gap-4">
        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          <p className="text-xs font-medium text-gray-500 mb-1">Gross Burn</p>
          <p className="text-2xl font-bold text-red-500">€{grossBurn.toLocaleString()}</p>
          <p className="text-xs text-gray-400 mt-1">Total monthly expenses</p>
        </div>
        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          <p className="text-xs font-medium text-gray-500 mb-1">Net Burn</p>
          <p className={`text-2xl font-bold ${netBurn <= 0 ? 'text-green-600' : 'text-amber-600'}`}>
            {netBurn <= 0 ? '-' : '+'}€{Math.abs(netBurn).toLocaleString()}
          </p>
          <p className="text-xs text-gray-400 mt-1">After MRR offset</p>
        </div>
        <div className="rounded-xl border border-amber-100 bg-amber-50 p-5 shadow-sm">
          <p className="text-xs font-medium text-amber-600 mb-1">Outstanding Invoices</p>
          <p className="text-2xl font-bold text-amber-700">€890</p>
          <p className="text-xs text-amber-500 mt-1">2 unpaid client invoices</p>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm flex items-center justify-between">
        <div>
          <p className="text-sm font-semibold text-gray-900">Payroll Obligations</p>
          <p className="text-xs text-gray-400 mt-0.5">Monthly fixed cost estimate</p>
        </div>
        <p className="text-lg font-bold text-gray-900">€4,200 <span className="text-xs text-gray-400 font-normal">/ mo</span></p>
      </div>
